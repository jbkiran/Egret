    ...
    return (
        <aside className="flex flex-col md:w-[250px] gap-3 sm:w-[35%]">
            <div className="md:w-[250px] sm:w-full rounded-2xl shadow-lg flex-1 overflow-hidden hidden sm:block">
                <Image
                    src={getPublicFileUrl('images',profileData.profileImage)}
                    alt="Profile"
                    width={250}
                    height={250}
                    // placeholder="blur"
            ...
            <div className="bg-[var(--color-foreground)] md:w-[250px] sm:w-full rounded-2xl shadow-lg flex-1 p-4 text-center flex flex-col relative">
                <h2 className="text-xl font-bold text-[var(--color-primary)]">{profileData.name}</h2>
                <p className="text-[var(--color-secondary)]">{profileData.title}</p>
                <hr className="border-t border-[var(--color-border-color)] w-full my-6" />
                ...
                <div className="relative sm:absolute sm:bottom-3 sm:left-3 sm:right-3">
                    <button
                    onClick={() => handleDownload(profileData.resumeUrl)}
                    disabled={isDownloading}
                    className="w-full flex justify-between text-white bg-[var(--color-accent)] hover:bg-[var(--color-accent-primary)] font-medium rounded-lg text-sm px-5 py-2.5 text-center items-center">
                        <label>{isDownloading ? 'Downloading...' : 'Download Resume'}</label>
